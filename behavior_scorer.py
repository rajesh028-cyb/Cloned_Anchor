...
FORMULA (per-turn contribution, accumulated):
  Score_t = 0.35·ΔUrgency + 0.25·PressureLex + 0.20·CredentialRepeat
          + 0.10·DelayTolerance + 0.10·PolitenessShift

TOTAL SESSION SCORE = mean(Score_1 … Score_t)  ∈ [0.0, 1.0]

THRESHOLDS:
  score >= 0.60  →  force EXTRACT  (high confidence scammer)
  score >= 0.40  →  prefer EXTRACT / CLARIFY
  score <  0.40  →  normal state rotation
